...
startButton.addEventListener('click',generateRandomCard());
...
function getRandomEnum(blurEnums) {
  if (blurEnums.some((Element) => Element === "all")) {
    const randomIndex = Math.floor(Math.random() * blurOptions.length);
    return blurOptions[randomIndex];
  } else {
